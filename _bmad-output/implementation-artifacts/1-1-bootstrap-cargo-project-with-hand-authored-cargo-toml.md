...
_Added 2026-04-22 by `bmad-code-review` (3-layer adversarial review: Blind Hunter + Edge Case Hunter + Acceptance Auditor; 13 of 19 raw findings dismissed as context-blind false positives or spec-explicit decisions; 1 finding initially dismissed but later **confirmed real** — see Correction below)._

_**Correction (2026-04-22, post-review):** Both Blind Hunter and Edge Case Hunter flagged `[target.'cfg(debug_assertions)'.dependencies]` as semantically broken. I dismissed both based on `cargo check` succeeding without errors. Re-running `cargo check` after the package-name rename revealed the warning Cargo had been emitting all along (suppressed in my earlier tail-only output): `Found 'debug_assertions' in target.'cfg(...)'.dependencies. This value is not supported... will not work as expected`. The reviewers were right; the spec's skeleton pattern is broken. Logged in `deferred-work.md` under "Review correction"._
...
- [x] [Review][Defer] **No explicit `[[bin]] name = "asteroids3d"` — binary inherits mixed-case package name** — Spec explicitly routes packaging/release-binary naming to Story 4.10 / Epic 10 polish. Leave as-is.
- [x] [Review][Defer] **🐛 `[target.'cfg(debug_assertions)'.dependencies]` does NOT strip bevy_egui from release builds (CORRECTION — initially dismissed)** — Cargo evaluates `cfg()` predicates in dependency tables before rustc-flag processing, so `debug_assertions` is treated as always-true; bevy_egui is pulled into every build. Recommended fix: feature-flag pattern (`[features] dev-tools = ["dep:bevy_egui"]`) when Story 1.5 first registers the egui plugin. Architecture.md skeleton may need an erratum. Full detail in `deferred-work.md`.
